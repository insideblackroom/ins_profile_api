...
@app.get("/file_v", response_class=FileResponse)
async def file_response():
    if os.path.exists("./stuff/reelsvideo.mp4"):
        return FileResponse("./stuff/reelsvideo.mp4")
    return {"Message": "File not exist"}

@app.get("/file_pic", response_class=FileResponse)
async def file_pic_response():
    if os.path.exists("./stuff/postphoto.jpg"):
        return FileResponse("./stuff/profilepic.jpg")
    return {"Message": "File not exist"}

@app.get("/file_album")
async def file_album_response():
    if glob("./album/*"):
        zipfiles(path="./album")
        return FileResponse("./zippedfiles.zip")
    return {"Message": "File not exist"}

@app.get("/file_post_pic")
async def file_post_pic_response():
    if os.path.exists("./stuff/postphoto.jpg"):
        return FileResponse("./stuff/postphoto.jpg")
    else:
        return {"Message": "File not exist"}

@app.get("/file_highlight")
async def file_highlight_response():
    if glob("./highlight/*"):
        zipfiles(path="./highlight", zip_name="highlightzippedfiles.zip")
...
def post_download(request: Request, data, client: Client):
    if data['media_type'] == 'reels':
        path =  client.video_download_by_url(url=data['post_video_url'], filename="reelsvideo", folder="./stuff")
        data['post_video_url'] = request.base_url._url + "file_v"
        return path
    elif data['media_type'] == 'Album':
        path = client.album_download_by_urls(urls=data['post_thumbnail_urls'], folder="./album")
        data.update({"post_zipped_image": request.base_url._url + "file_album"})
        return path
    else:
        path = client.photo_download_by_url(url=data['post_thumbnail_url'], filename="postphoto", folder="./stuff")
        data['post_thumbnail_url'] = request.base_url._url + "file_post_pic"
        return path
...
async def highlights(
    ...
    for user, passwd in user_pass.items():
        ...
        if r == 0:
            ...
            listdir = glob("*.mp4", root_dir="./highlight")
            if listdir:
                for file in listdir:
                    os.remove(f"./highlight/{file}")
            for k in highlight_data.keys():
                h_info = client.highlight_info(highlight_data[k]["pk"]).model_dump() 
                for story in h_info['items']:
                    if story['video_url']:
                        client.video_download_by_url(str(story['video_url']), folder="./highlight")
                    else:
                        client.photo_download_by_url(str(story['thumbnail_url']), folder="./highlight")
                highlight_data[k].update({"highlights_zipped_url": request.base_url._url + "file_highlight"})
